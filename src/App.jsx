...
export default function App() {
  ...
  var fltSt  = useState({s:"todos",src:"todos",q:"",fDesde:"",fHasta:"",to_origen:"todos",to_destino:"todos"}); var flt=fltSt[0],setFlt=fltSt[1];
  ...
  var fltd = results.filter(function(r) {
    ...
    return (flt.s==="todos"||r.status===flt.s)&&(flt.src==="todos"||r.source===flt.src)
      &&(!flt.fDesde||(fechaToKey(r.fecha,"dia")||"")>=flt.fDesde)&&(!flt.fHasta||(fechaToKey(r.fecha,"dia")||"")<=flt.fHasta)
        ...
        {tab==="dashboard" && results.length>0 && (
          <div className="vge-fade">
            ...
            <div style={{ background:T.bgSurface, borderRadius:T.r12, border:"1px solid "+T.borderFaint,
              ...
              {dashFilt.mode==="dia" && (
                <input type="date" value={dashFilt.val}
                  min={dDias[0]||undefined} max={dDias[dDias.length-1]||undefined}
                  style={Object.assign({},selSt,{padding:"7px 10px"})}
                  onChange={function(e){var v=e.target.value;setDashFilt(function(p){return Object.assign({},p,{val:v});});}} />
              )}
              {dashFilt.mode==="rango" && (
                <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                  <input type="date" value={dashFilt.from}
                    min={dDias[0]||undefined} max={dDias[dDias.length-1]||undefined}
                    style={Object.assign({},selSt,{padding:"7px 10px"})}
                    onChange={function(e){var v=e.target.value;setDashFilt(function(p){return Object.assign({},p,{from:v});});}} />
                  <span style={{ fontSize:11,color:T.textMuted }}>→</span>
                  <input type="date" value={dashFilt.to}
                    min={dashFilt.from||dDias[0]||undefined} max={dDias[dDias.length-1]||undefined}
                    style={Object.assign({},selSt,{padding:"7px 10px"})}
                    onChange={function(e){var v=e.target.value;setDashFilt(function(p){return Object.assign({},p,{to:v});});}} />
                </div>
              )}
        ...
        {tab==="resultados" && (
          <div className="vge-fade">
            ...
            <div style={{ display:"flex",gap:10,marginBottom:16,flexWrap:"wrap",alignItems:"center" }}>
              ...
              {/* Filtro fecha: rango con calendario (Desde / Hasta) */}
              <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                <input type="date" value={flt.fDesde} title="Desde"
                  min={dDias[0]||undefined} max={dDias[dDias.length-1]||undefined}
                  style={Object.assign({},selSt,{padding:"7px 10px"})}
                  onChange={function(e){var v=e.target.value;setFlt(function(p){return Object.assign({},p,{fDesde:v});});}} />
                <span style={{ fontSize:11,color:T.textMuted }}>→</span>
                <input type="date" value={flt.fHasta} title="Hasta"
                  min={flt.fDesde||dDias[0]||undefined} max={dDias[dDias.length-1]||undefined}
                  style={Object.assign({},selSt,{padding:"7px 10px"})}
                  onChange={function(e){var v=e.target.value;setFlt(function(p){return Object.assign({},p,{fHasta:v});});}} />
                {(flt.fDesde||flt.fHasta) && (
                  <button onClick={function(){setFlt(function(p){return Object.assign({},p,{fDesde:"",fHasta:""});});}}
                    style={{ padding:"6px 8px",background:"transparent",border:"1px solid "+T.borderLight,
                      borderRadius:T.r6,fontSize:10,cursor:"pointer",color:T.textMuted }} title="Limpiar fechas">✕</button>
                )}
              </div>
              {/* Filtro tipo origen */}
