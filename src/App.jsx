    ...
    <div style={{ fontFamily:"system-ui,sans-serif", background:"#f1f5f9", position:"relative", minHeight:600 }}>
      ...
      <div style={{ padding:"18px 18px 40px", maxWidth:1200, margin:"0 auto" }}>
        ...
        {tab==="dashboard" && results.length===0 && (
          <Empty msg="Selecciona una fecha y valida para ver el dashboard" cta="Ir a Inicio" onCta={function(){setTab("inicio");}} />
        )}
        {tab==="dashboard" && results.length>0 && (
          <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:10, marginBottom:14 }}>
              {[{l:"Total",v:stT,c:"#334155",bg:"#f8fafc"},{l:"Válidas",v:stV,c:"#15803d",bg:"#f0fdf4"},{l:"Sospechosas",v:stS,c:"#b45309",bg:"#fffbeb"},{l:"Anomalías",v:stA,c:"#b91c1c",bg:"#fef2f2"},{l:"Autorizadas",v:stAu,c:"#5b21b6",bg:"#f5f3ff"}].map(function(card,i){
                return (
                  <div key={i} style={{ background:card.bg,borderRadius:11,padding:"12px 14px",border:"1px solid "+card.c+"18" }}>
                    <div style={{ fontSize:26,fontWeight:600,color:card.c,lineHeight:1 }}>{card.v}</div>
                    <div style={{ fontSize:11,color:card.c,opacity:0.8,marginTop:3 }}>{card.l}</div>
                    {i>0&&stT>0&&<div style={{ fontSize:10,color:card.c,opacity:0.5 }}>{Math.round(card.v/stT*100)}%</div>}
                  </div>
                );
              })}
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:14 }}>
              <div style={{ background:"#ffffff",borderRadius:11,padding:14,border:"1px solid #e2e8f0" }}>
                ...
                <ResponsiveContainer width="100%" height={155}><PieChart><Pie data={pieData} cx="50%" cy="50%" innerRadius={36} outerRadius={60} dataKey="v" nameKey="n">{pieData.map(function(e,i){return <Cell key={i} fill={e.f}/>;})}</Pie><Tooltip/></PieChart></ResponsiveContainer>
                <div style={{ display:"flex",gap:7,justifyContent:"center",flexWrap:"wrap" }}>{pieData.map(function(e,i){return <div key={i} style={{ display:"flex",gap:4,alignItems:"center",fontSize:9 }}><div style={{ width:7,height:7,borderRadius:2,background:e.f }}/>{e.n}:{e.v}</div>;})}</div>
              </div>
              <div style={{ background:"#ffffff",borderRadius:11,padding:14,border:"1px solid #e2e8f0" }}>
                <div style={{ fontWeight:500,fontSize:12,marginBottom:8 }}>Por fuente</div>
                <ResponsiveContainer width="100%" height={155}><BarChart data={barData}><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}}/><Tooltip/><Bar dataKey="Válidas" fill="#22c55e" radius={[3,3,0,0]}/><Bar dataKey="Sospechosas" fill="#f59e0b" radius={[3,3,0,0]}/><Bar dataKey="Anomalías" fill="#ef4444" radius={[3,3,0,0]}/></BarChart></ResponsiveContainer>
              </div>
              <div style={{ background:"#ffffff",borderRadius:11,padding:14,border:"1px solid #e2e8f0" }}>
                <div style={{ fontWeight:500,fontSize:12,marginBottom:8 }}>Tipo de Origen</div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:14 }}>
              <div style={{ background:"#ffffff",borderRadius:11,padding:14,border:"1px solid #e2e8f0" }}>
                ...
                <div style={{ fontSize:10,color:"#64748b",marginBottom:8 }}>Clasifica según Tipo Origen · Ajusta en ⚙ Reglas</div>
                {cmdTipo.length===0?<div style={{ textAlign:"center",padding:24,fontSize:11,color:"#64748b" }}>Sin clasificaciones</div>
                  :<ResponsiveContainer width="100%" height={Math.max(cmdTipo.length*30+20,140)}><BarChart data={cmdTipo} layout="vertical"><XAxis type="number" tick={{fontSize:9}} allowDecimals={false}/><YAxis type="category" dataKey="tipo" tick={{fontSize:9}} width={145}/><Tooltip/><Bar dataKey="n" name="Guías" fill="#6366f1" radius={[0,4,4,0]}/></BarChart></ResponsiveContainer>}
              </div>
              <div style={{ background:"#ffffff",borderRadius:11,padding:14,border:"1px solid #e2e8f0" }}>
                <div style={{ fontWeight:500,fontSize:12,marginBottom:2 }}>Tipo de operación (WS)</div>
                <div style={{ fontSize:10,color:"#64748b",marginBottom:8 }}>TR, Ecommerce, Cambio Catálogo, Cuenta Concentradora</div>
                {wsTipo.length===0?<div style={{ textAlign:"center",padding:24,fontSize:11,color:"#64748b" }}>Sin clasificaciones</div>
            ...
            {(stS+stA)>0&&(
              <div style={{ padding:12,background:"#fef2f2",borderRadius:10,border:"1px solid #fecaca",display:"flex",gap:10,alignItems:"center",flexWrap:"wrap" }}>
                <span style={{ fontSize:13,color:"#b91c1c",fontWeight:500 }}>⚠ {stS+stA} guías requieren atención</span>
                {stA>0&&<button onClick={function(){setFlt(function(f){return Object.assign({},f,{s:"anomalia"});});setTab("resultados");}} style={{ padding:"6px 12px",background:"#dc2626",color:"white",border:"none",borderRadius:6,fontSize:11,cursor:"pointer" }}>Ver {stA} anomalías →</button>}
                {stS>0&&<button onClick={function(){setFlt(function(f){return Object.assign({},f,{s:"sospechosa"});});setTab("resultados");}} style={{ padding:"6px 12px",background:"#d97706",color:"white",border:"none",borderRadius:6,fontSize:11,cursor:"pointer" }}>Ver {stS} sospechosas →</button>}
              </div>
            )}
          </div>
        )}
